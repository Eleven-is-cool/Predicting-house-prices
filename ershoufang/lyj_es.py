# -*- coding: utf-8 -*-
"""
Created on Mon Oct 21 20:37:42 2019
...
def get_list_page_url(city):
    ...
    start_url = "https://{}.leyoujia.com/esf/".format(city)
    headers =  {
    ...
    try:
        ...
        total_num =  int(doc(".hd_title em").text())
...
def get_detail_page_url(page_url, city):
    pre_url =  "https://{}.leyoujia.com".format(city)
    global detail_list
    headers =  {
        'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/76.0.3809.132 Safari/537.36',
        'Referer': 'https://you.leyoujia.com/bj'
    }
    ...
    try:
        ...
        i = 0
        detail_urls = list()
        for item in doc(".list-box .item.clearfix").items():
            
            i += 1
            if i == 21:
                break
            child_item = item(".text .tit a")
            #print(child_item)
            if child_item == None:
                i -= 1
...
def detail_page_parser(res):
    ...
    for detail_url in detail_urls:
        try:
            ...
            city = doc(".crumbs a").eq(1).text().strip()
            city = city[:-3]            
            unit_price = doc(".intro-box1.clearfix p").text()
            unit_price = unit_price[0:unit_price.index("元")]#去掉单位
            unit_price = unit_price[2:]
            title = doc("h1").text()
            communityName = doc(".intro-box3 .clearfix .fl.xqname").text()
            area1 = doc(".area-select .area-cur").eq(0).text().strip()
            area2 = doc(".area-select .area-cur").eq(1).text().strip()
            area = area1[:-3] + area2[:-3]
            floor = doc(".xq-box.xq-box2 .cont .w2").eq(4).text()
            floor = floor[2:]
            direction = doc(".xq-box.xq-box2 .cont .w2").eq(0).text() 
            direction = direction[2:]
            ReleaseTime = doc(".xq-box.xq-box2 .cont .w2").eq(5).text()
            ReleaseTime = ReleaseTime[4:]
            support = doc(".comm-box.fy-intro").text().replace('\n',' ')        
            url = detail_url
...
def save_data(data,filename):
    with open("Lyj_"+filename+"_ershou.json", 'w', encoding="utf-8") as f:
...
def main():
    city_list = ['dongguan']#['foshan','zhuhai','guangzhou','shenzhen','dongguan','zhongshan','huizhou','jiangmen','zhaoqing']
